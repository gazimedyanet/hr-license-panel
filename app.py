...
def generate_license_key(hw_id: str, expires_at: str) -> str:
    """Key, expires_at tarihine göre üretilir — days'e göre değil"""
...
def index():
    ...
    conn.close()
    return render_template_string(PANEL_HTML, licenses=licenses, stats=stats, now=now)
...
def create():
    ...
    key = generate_license_key(hw_id, expires)
    conn = get_db()
    try:
...
def extend(lid):
    ...
    if lic:
        ...
        if cur < datetime.now():
            cur = datetime.now()
        new_exp = cur + timedelta(days=days)          # Mevcut tarihten days ekle
        new_key = generate_license_key(lic['hw_id'], new_exp.isoformat())
...
<head>
<meta charset="UTF-8">
<title>Lisans Paneli — Gazi Medya</title>
<style>
...
nav h1{color:#fff;font-size:17px}
nav a{color:#64748b;text-decoration:none;font-size:13px}
nav a:hover{color:#fff}
.wrap{max-width:1350px;margin:0 auto;padding:24px}
.stats{display:grid;grid-template-columns:repeat(4,1fr);gap:12px;margin-bottom:20px}
.stat{background:#fff;border-radius:10px;padding:18px;border:1px solid #e5e7eb;text-align:center}
.stat b{font-size:30px;display:block;font-weight:800}
.stat span{font-size:11px;color:#9ca3af;text-transform:uppercase;letter-spacing:1px}
...
input,select{width:100%;padding:8px 10px;border:1px solid #e5e7eb;border-radius:6px;font-size:13px;outline:none}
input:focus,select:focus{border-color:#1677ff}
.btn{padding:7px 14px;border:none;border-radius:6px;cursor:pointer;font-size:12px;font-weight:600;display:inline-flex;align-items:center;gap:5px}
.bp{background:#1677ff;color:#fff}
.bd{background:#dc2626;color:#fff}
.bw{background:#d97706;color:#fff}
...
.key{font-family:monospace;font-size:11px;background:#f8fafc;padding:3px 7px;
     border-radius:4px;cursor:pointer;display:inline-block;max-width:220px;
...
.ba{background:#dcfce7;color:#16a34a}
...
.mbox{background:#fff;border-radius:12px;padding:24px;width:320px;
  box-shadow:0 8px 32px rgba(0,0,0,.2)}
.mbox h3{font-size:15px;font-weight:700;margin-bottom:16px;color:#1a1f2e;
  display:flex;justify-content:space-between;align-items:center}
.mbox h3 button{background:none;border:none;font-size:18px;cursor:pointer;
  color:#9ca3af;line-height:1;padding:0}
.mbox h3 button:hover{color:#374151}
.mbox label{font-size:11px;color:#6b7280;display:block;margin-bottom:4px}
.mbox input,.mbox select{width:100%;padding:8px 10px;border:1px solid #e5e7eb;
  border-radius:6px;font-size:13px;outline:none;margin-bottom:12px}
.mbox input:focus,.mbox select:focus{border-color:#1677ff}
...
<nav>
  <h1>🔐 Gazi Medya — HR Lisans Paneli</h1>
...
<div class="wrap">
  ...
  <div class="card">
    ...
    <form method="POST" action="/create">
      <div class="grid3">
        ...
        <div><label>Müşteri / Firma Adı</label>
        ...
        <div><label>Not (ödeme, sipariş no vb.)</label>
  ...
  <!-- Lisanslar -->
  <div class="card">
    <h2>📋 Lisans Listesi ({{licenses|length}} kayıt)</h2>
    <table>
      <tr>
      ...
      <tr>
        ...
        <td>
          <b>{{l.customer_name or '—'}}</b><br>
          <small style="color:#9ca3af">{{l.customer_email or ''}}</small>
        </td>
        <td>
          <span class="key" title="Kopyalamak için tıkla"
                onclick="copyText('{{l.license_key}}')">{{l.license_key}}</span>
        </td>
        <td>
          <span class="key" onclick="copyText('{{l.hw_id}}')">{{l.hw_id[:20]}}...</span>
        </td>
        <td>{{l.expires_at[:10]}}</td>
        <td style="color:#9ca3af;font-size:11px">{{l.last_seen[:16] if l.last_seen else 'Henüz yok'}}</td>
        <td style="text-align:center;font-weight:700">{{l.verify_count or 0}}</td>
        <td>
          {% if l.is_revoked %}<span class="badge br">İptal</span>
          {% elif l.expires_at < now %}<span class="badge bo">Dolmuş</span>
        ...
        <td>
          <div class="acts">
            ...
            <button type="button" class="btn bg"
              onclick="openModal('uzat{{l.id}}')">Uzat</button>
            <!-- DÜZENLE -->
            <button type="button" class="btn bv"
              onclick="openModal('duzenle{{l.id}}')">Düzenle</button>
            <!-- İPTAL/AKTİFLEŞTİR -->
            {% if not l.is_revoked %}
            <button type="button" class="btn bw"
              onclick="openModal('iptal{{l.id}}')">İptal</button>
            {% else %}
            <form method="POST" action="/restore/{{l.id}}" style="display:inline">
              <button type="submit" class="btn bg">Aktifleştir</button>
            </form>
            {% endif %}
            <!-- SİL -->
            <form method="POST" action="/delete/{{l.id}}" style="display:inline"
                  onsubmit="return confirm('Lisans kalıcı silinecek!')">
              <button type="submit" class="btn bd" style="padding:5px 8px;font-size:11px">✕ Sil</button>
            </form>
          </div>
        </td>
      </tr>
      ...
      <!-- UZAT MODAL -->
      <div id="uzat{{l.id}}" class="modal" onclick="if(event.target===this)closeModal(this.id)">
        <div class="mbox">
          <h3>Süre Uzat <button onclick="closeModal('uzat{{l.id}}')">✕</button></h3>
      ...
      <!-- DÜZENLE MODAL -->
      <div id="duzenle{{l.id}}" class="modal" onclick="if(event.target===this)closeModal(this.id)">
        <div class="mbox">
          <h3>Müşteriyi Düzenle <button onclick="closeModal('duzenle{{l.id}}')">✕</button></h3>
          <form method="POST" action="/edit/{{l.id}}">
            ...
            <label>Not</label>
            <input name="notes" value="{{l.notes or ''}}">
      ...
      <!-- İPTAL MODAL -->
      {% if not l.is_revoked %}
      <div id="iptal{{l.id}}" class="modal" onclick="if(event.target===this)closeModal(this.id)">
        <div class="mbox">
          <h3>Lisansı İptal Et <button onclick="closeModal('iptal{{l.id}}')">✕</button></h3>
...
if __name__ == '__main__':
    ...
    print(f"Gazi Medya HR Lisans Paneli v4 — port {port}")
